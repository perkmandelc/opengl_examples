...
PFNGLTEXIMAGE2DMULTISAMPLEPROC glTexImage2DMultisample_f;
PFNGLRENDERBUFFERSTORAGEMULTISAMPLEPROC glRenderbufferStorageMultisample_f;
PFNGLBLITFRAMEBUFFERPROC glBlitFramebuffer_f;
...
static void create_fbo(int *fbo_texture_handle, int *fbo_handle, int num_msaa_samples) {
    int rbo_handle = -1;
    ...
    if (num_msaa_samples > 1) {
        GL_CHECK(glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, *fbo_texture_handle));
        GL_CHECK(glTexImage2DMultisample_f(GL_TEXTURE_2D_MULTISAMPLE, num_msaa_samples,
                                           GL_RGB,
                                           window_width, window_height,
                                           GL_TRUE));
        GL_CHECK(glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
        GL_CHECK(glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
        GL_CHECK(glFramebufferTexture2D_f(
                     GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                     GL_TEXTURE_2D_MULTISAMPLE, *fbo_texture_handle, 0));
        GL_CHECK(glRenderbufferStorageMultisample_f(GL_RENDERBUFFER,
                                                    num_msaa_samples,
        ...
        GL_CHECK(glRenderbufferStorage_f(GL_RENDERBUFFER,
                                         GL_DEPTH_COMPONENT32F,
                                         window_width, window_height));
    }
...
int main(int argc, char *argv[]) {
    ...
    int tmp_fbo_handle = -1;
    int tmp_fbo_texture_handle = -1;

    int num_msaa_samples = 8;
    
    { // setup the fbo
        create_fbo(&fbo_texture_handle, &fbo_handle, num_msaa_samples);
        create_fbo(&tmp_fbo_texture_handle, &tmp_fbo_handle, 1);
    }
    ...
    while (1) {
...

        {
            ...
            GL_CHECK(glBindFramebuffer_f(GL_READ_FRAMEBUFFER, fbo_handle));
            GL_CHECK(glBindFramebuffer_f(GL_DRAW_FRAMEBUFFER, tmp_fbo_handle));
            GL_CHECK(glBlitFramebuffer_f(0, 0, window_width, window_height,
        ...
        {
            GL_CHECK(glActiveTexture_f(GL_TEXTURE0 + tex_unit_offset));
            GL_CHECK(glBindTexture(GL_TEXTURE_2D, tmp_fbo_texture_handle));
